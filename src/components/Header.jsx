    ...
    <header className="border-b border-gray-200 dark:border-gray-800 sticky top-0 bg-white/80 dark:bg-gray-900/80 backdrop-blur z-50">
      <div className="container py-3 flex items-center justify-between">
        ...
        <nav className="hidden md:flex items-center gap-6">
          <NavLink to="/" className={({ isActive }) => isActive ? 'text-brand font-semibold' : 'hover:text-brand'}>
            Home
          </NavLink>
          <NavLink to="/products" className={({ isActive }) => isActive ? 'text-brand font-semibold' : 'hover:text-brand'}>
            Products
          </NavLink>
          <NavLink to="/cart" className={({ isActive }) => isActive ? 'text-brand font-semibold' : 'hover:text-brand'}>
            Cart
          </NavLink>
        </nav>
      ...
      {open && (
        <div className="md:hidden border-t border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-900">
          <div className="container py-2 flex flex-col gap-2">
            ...
            <NavLink to="/products" onClick={() => setOpen(false)} className="py-2">
              Products
            </NavLink>
            <NavLink to="/cart" onClick={() => setOpen(false)} className="py-2">
              Cart
            </NavLink>
          </div>
        </div>
